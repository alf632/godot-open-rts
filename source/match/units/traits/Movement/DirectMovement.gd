...
func _physics_process(delta):
	...
	if not piloted or not _dir_synchronizer.is_multiplayer_authority() and not multiplayer.is_server():
		return
	...
	if not multiplayer.is_server():
		...
		# players have authority over their inputs. but the server can also be a player
