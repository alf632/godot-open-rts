...
func _calculate_blueprint_position_validity():
	...
	var placement_validity = Utils.Match.UnitUtils.Placement.validate_agent_placement_position(
	...
	if placement_validity == Utils.Match.UnitUtils.Placement.COLLIDES_WITH_AGENT:
		return BlueprintPositionValidity.COLLIDES_WITH_OBJECT
	if placement_validity == Utils.Match.UnitUtils.Placement.NOT_NAVIGABLE:
