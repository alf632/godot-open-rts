...
func _ready():
	...
	_player.changed.connect(_on_player_data_changed)
...
func _try_fulfilling_resource_requests_according_to_priorities():
	...
	for priority in [
		...
		while (
			...
			and _player.has_resources(_resource_requests[priority].front()["resources"])
		):
		...
		if (
			not _resource_requests[priority].is_empty()
			and not _player.has_resources(_resource_requests[priority].front()["resources"])
		):
			break
